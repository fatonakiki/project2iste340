...
function CoopTable({ coopTable }) {
    ...
    const totalPages = Math.ceil(coopTable.length / itemsPerPage);
    ...
    //slice the coopTable object according to on which page we are and how many items we want per page
    const visibleItems = coopTable.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
    return (
        <>
                    ...
                    <Table.Body style={{ maxHeight: '200px', overflowY: 'auto' }}>
                        {visibleItems.map((employer, index) => (
                            <Table.Row key={index}>
                                ...
                                <Table.Cell>{employer.term}</Table.Cell>
                            </Table.Row>
                        ))}
